...
def main(stdscr):
    ...
    def draw_status():
        ...
        status.addstr(0, 2, f"lat: {round(globals.gps.lat,2)} lon: {round(globals.gps.lon,2)} netw seen: {globals.set_and_calc_networks()} CH: {globals.current_channel} TO rstL: {globals.scans_before_reset}".ljust(cols - 7))
